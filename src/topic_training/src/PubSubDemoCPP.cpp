...
auto main(int argc, char **argv) -> int
{
  /* Initialize ROS and specify the name of the node 
     ...
     header. */
  ros::init(argc, argv, "PubSubDemoCPP");
  ...
  while (ros::ok())
  {
    /* Declare the message as the type we specified when 
       we initialized the Publisher (std_msgs::String). */
    std_msgs::String msg;
    ...
    /* Use the function sleep() for the time remaining 
       until we reach our 10Hz publish rate. */
    loop_rate.sleep();
